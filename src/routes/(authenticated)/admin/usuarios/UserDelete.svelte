  ...
  <Modal title="Eliminación de organización" close={() => (isOpen = false)}>
    <section class="mb-6">
      <p class="mb-2 text-base leading-relaxed text-gray-500 dark:text-gray-400">
        ...
        <b>{props.row?.firstName} {props.row?.lastName || '(sin apellido)'}</b>
        cuyo email es <b>{props.row?.email}</b>? El usuario se encuentra
        {#if props.row?.isActive}
          <b class="text-green-500">activo</b>.
        {:else}
          <b class="text-red-500">inactivo</b>.
        {/if}
      </p>
    ...
    >
      ...
      <div class="rounded-sm border {isConfirmedErr ? 'border-red-400' : 'border-gray-300'}">
        ...
        >
          Confirmo que deseo eliminar al usuario.
        </Checkbox>
      </div>
      {#if isConfirmedErr}
